...
links = [] 
titles = [] 
time_uploaded = []  import requests
from bs4 import BeautifulSoup
from fake_useragent import UserAgent  # used to create fake user agent for requests
import pandas as pd
import os
from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer
import nltk
...
ua = UserAgent()
userAgent = ua.random
headers = {'User-Agent': userAgent}  # FIXED header key here
page = requests.get(url, headers=headers)
soup = BeautifulSoup(page.content, 'html.parser')
...
# Fix get_sentiment to accept actual text
